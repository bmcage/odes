...
initx = asarray([1, 0.1], float)
initxp = asarray([initx[1], -k/m*initx[0]], float)
...
result = solver.run_solver(asarray([0., 1., 2.],float), initx, initxp)
...
#continue the solver
result = solver.run_solver(asarray([result[1][-1], result[1][-1]+1],float), 
