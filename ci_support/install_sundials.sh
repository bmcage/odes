...
cd sundials_build &&
    cmake -DCMAKE_INSTALL_PREFIX=$SUNDIALS_DIR -DLAPACK_ENABLE=ON -DSUNDIALS_INDEX_SIZE="$INDEX_SIZE" -DSUNDIALS_PRECISION="$PRECISION" -DBUILD_ARKODE:BOOL=OFF  -DEXAMPLES_INSTALL:BOOL=OFF ../$SUNDIALS &&
    make && make install
