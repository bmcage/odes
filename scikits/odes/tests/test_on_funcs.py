...
class TestOn(TestCase):
    ...
    def test_cvode_tstopfn_notstop(self):
        ...
        n = 0
        tspan = np.arange(0, t_end1 + 1, 1.0, np.float)
        solver = ode('cvode', rhs_fn, tstop=Y1, ontstop=ontstop_va,
                     old_api=False)
    ...
    def test_cvode_tstopfn(self):
        ...
        n = 0
        tspan = np.arange(0, t_end2 + 1, 1.0, np.float)
        solver = ode('cvode', rhs_fn, tstop=Y1,
                     old_api=False)
        soln = solver.solve(tspan, y0)
    ...
    def test_cvode_tstopfn_stop(self):
        ...
        n = 0
        tspan = np.arange(0, t_end2 + 1, 1.0, np.float)
        solver = ode('cvode', rhs_fn, tstop=Y1, ontstop=ontstop_vb,
                     old_api=False)
    ...
    def test_cvode_tstopfn_test(self):
        ...
        n = 0
        tspan = np.arange(0, t_end2 + 1, 1.0, np.float)
        solver = ode('cvode', rhs_fn, tstop=Y1, ontstop=ontstop_vc,
                     old_api=False)
